...
`POST /jobs/{job_id}` accepts optional `num_speakers`, `speaker_gap_seconds` and `batch_size` (`1` through `16`, default `8`). Worker logs diarization time, Whisper time and peak VRAM for every job.
